...
export default {
  ...
  mounted() {},
  methods: {
    histval(deviceId, startTime, endTime) {
    ...
    historyChange() {
      ...
      this.histval(this.deviceId, this.startTime, this.endTime)
    },
    aWeek() {
      // 获取上一周的开始和结束时间
      var myDate = new Date()
      ...
      this.histval(this.deviceId, new Date(beginTime).getTime(), new Date(endTime).getTime())
    },
    aMonth() {
      // 获取上一月的开始和结束时间
      var myDate = new Date()
